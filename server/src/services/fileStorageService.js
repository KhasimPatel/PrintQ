 ...
 *
 * publicId is REQUIRED on Order.files in the schema — both branches below
 * must always return one, even in mock mode, or Order.create() throws a
 * validation error on every order. The mock branch uses the locally
 * generated storedName as its publicId, since there's no real Cloudinary
 * asset to reference.
 *
 * @param {Object} file - multer file object (buffer, originalname, mimetype, size)
 * @returns {Promise<{ fileUrl: string, publicId: string, fileName: string, fileSize: number }>}
 */
...
/**
 * Deletes a file from Cloudinary using its publicId. No-op-safe to call
 * even if the file was stored via mock storage in dev — ensureCloudinaryConfigured()
 * will throw clearly in that case rather than silently doing nothing, so
 * callers should only invoke this when USE_MOCK_STORAGE=false.
 */
